...
const FoodCart = () => {
  ...
  const UpdateHandler = (product) => {
    ...
    if (x == -1) {
      ...
      conpyUser.cart[x] = {
        product,
        quantity: conpyUser.cart[x].quantity + 1,
      };
    }
  ...
  const renderProducts =
    ...
    product.map((product) => {
      return (
        ...
        >
          <div className="" onClick={() => setSelectedProduct(product)}>
            <div className="">
          ...
          <div className=" flex items-center absolute bottom-4  gap-10  ">
            <h3 className="text-[15px] w-20 font-medium text-[#323232]">
              ₹{product?.price}
            </h3>
            {users?.isAdmin ? (
              <button className="py-2 px-6 bg-[#ff806d67] border border-[#ff3c00] text-[#ff3300] font-semibold rounded-2xl active:scale-95 transition">
                Delete
              </button>
            ) : (
              <button
...
/>
    ...
    transition-opacity duration-300"
        >
          <div
            className="bg-white w-[600px] lg:w-[650px] h-[650px] lg:h-[330px] mt-44 lg:mt-0 p-6 rounded-xl flex gap-6 relative
      transform transition-all duration-300 scale-95 animate-[popup_0.35s_ease]"
          >
            {/* Close Button */}
            <button
            ...
            <div className="mt-10 lg:mt-0 flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex flex-col justify-between mt-12 lg:mt-2">
                ...
                <div className="flex items-center mt-20 lg:mt-0  justify-evenly">
                  <p className="text-[35px] lg:text-[28px] mt-4 font-semibold">
                    ₹{selectedProduct.price}
                  </p>

                  {users && users.isAdmin ? (
                    <button className="bg-red-500 text-white px-11 lg:px-3 lg:py-2 py-3 rounded ml-20 mt-5">
                      Delete
                    </button>
                  ) : (
                    <button
